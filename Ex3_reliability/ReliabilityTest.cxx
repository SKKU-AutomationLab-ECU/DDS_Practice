// Copyright 2016 Proyectos y Sistemas de Mantenimiento SL (eProsima).
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

/*!
 * @file ReliabilityTest.cpp
 * This source file contains the implementation of the described types in the IDL file.
 *
 * This file was generated by the tool fastddsgen.
 */

#ifdef _WIN32
// Remove linker warning LNK4221 on Visual Studio
namespace {
char dummy;
}  // namespace
#endif  // _WIN32

#include "ReliabilityTest.h"

#include <fastcdr/Cdr.h>


#include <fastcdr/exceptions/BadParamException.h>
using namespace eprosima::fastcdr::exception;

#include <utility>




TestData::TestData()
{
}

TestData::~TestData()
{
}

TestData::TestData(
        const TestData& x)
{
    m_timestamp = x.m_timestamp;
    m_sequence_number = x.m_sequence_number;
    m_message = x.m_message;
    m_is_critical = x.m_is_critical;
}

TestData::TestData(
        TestData&& x) noexcept
{
    m_timestamp = x.m_timestamp;
    m_sequence_number = x.m_sequence_number;
    m_message = std::move(x.m_message);
    m_is_critical = x.m_is_critical;
}

TestData& TestData::operator =(
        const TestData& x)
{

    m_timestamp = x.m_timestamp;
    m_sequence_number = x.m_sequence_number;
    m_message = x.m_message;
    m_is_critical = x.m_is_critical;
    return *this;
}

TestData& TestData::operator =(
        TestData&& x) noexcept
{

    m_timestamp = x.m_timestamp;
    m_sequence_number = x.m_sequence_number;
    m_message = std::move(x.m_message);
    m_is_critical = x.m_is_critical;
    return *this;
}

bool TestData::operator ==(
        const TestData& x) const
{
    return (m_timestamp == x.m_timestamp &&
           m_sequence_number == x.m_sequence_number &&
           m_message == x.m_message &&
           m_is_critical == x.m_is_critical);
}

bool TestData::operator !=(
        const TestData& x) const
{
    return !(*this == x);
}

/*!
 * @brief This function sets a value in member timestamp
 * @param _timestamp New value for member timestamp
 */
void TestData::timestamp(
        uint64_t _timestamp)
{
    m_timestamp = _timestamp;
}

/*!
 * @brief This function returns the value of member timestamp
 * @return Value of member timestamp
 */
uint64_t TestData::timestamp() const
{
    return m_timestamp;
}

/*!
 * @brief This function returns a reference to member timestamp
 * @return Reference to member timestamp
 */
uint64_t& TestData::timestamp()
{
    return m_timestamp;
}


/*!
 * @brief This function sets a value in member sequence_number
 * @param _sequence_number New value for member sequence_number
 */
void TestData::sequence_number(
        uint32_t _sequence_number)
{
    m_sequence_number = _sequence_number;
}

/*!
 * @brief This function returns the value of member sequence_number
 * @return Value of member sequence_number
 */
uint32_t TestData::sequence_number() const
{
    return m_sequence_number;
}

/*!
 * @brief This function returns a reference to member sequence_number
 * @return Reference to member sequence_number
 */
uint32_t& TestData::sequence_number()
{
    return m_sequence_number;
}


/*!
 * @brief This function copies the value in member message
 * @param _message New value to be copied in member message
 */
void TestData::message(
        const std::string& _message)
{
    m_message = _message;
}

/*!
 * @brief This function moves the value in member message
 * @param _message New value to be moved in member message
 */
void TestData::message(
        std::string&& _message)
{
    m_message = std::move(_message);
}

/*!
 * @brief This function returns a constant reference to member message
 * @return Constant reference to member message
 */
const std::string& TestData::message() const
{
    return m_message;
}

/*!
 * @brief This function returns a reference to member message
 * @return Reference to member message
 */
std::string& TestData::message()
{
    return m_message;
}


/*!
 * @brief This function sets a value in member is_critical
 * @param _is_critical New value for member is_critical
 */
void TestData::is_critical(
        bool _is_critical)
{
    m_is_critical = _is_critical;
}

/*!
 * @brief This function returns the value of member is_critical
 * @return Value of member is_critical
 */
bool TestData::is_critical() const
{
    return m_is_critical;
}

/*!
 * @brief This function returns a reference to member is_critical
 * @return Reference to member is_critical
 */
bool& TestData::is_critical()
{
    return m_is_critical;
}


// Include auxiliary functions like for serializing/deserializing.
#include "ReliabilityTestCdrAux.ipp"

